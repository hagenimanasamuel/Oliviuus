...
const HoverModal = ({ content, position, cardRect, onClose, onPlay, onAddToList, onMoreInfo, onMouseEnter, onMouseLeave }) => {
  ...
  const getGenres = () => {
    ...
    return content.genre || t('contentdetail.metadata.movie', 'Movie');
  };
  ...
  const getModalStyle = () => {
    ...
    if (redirecting) {
      // ZOOM-IN animation for redirect (changed from zoom-out)
      return {
        left: position.left,
        top: position.top,
        width: modalWidth,
        height: modalHeight,
        transform: 'scale(1.5)', // Zoom in to 150% (changed from scale(0.1))
        opacity: 0,
        borderRadius: '12px', // Keep rectangular shape (changed from '50%')
          ...
          {content.trailer ? (
            <>
              ...
              <div className={`absolute inset-0 transition-opacity duration-500 ${
                ...
                <button
                  onClick={toggleMute}
                  className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 bg-black/70 hover:bg-black/90 text-white rounded-full p-1.5 sm:p-2 transition-all duration-200 backdrop-blur-sm transform hover:scale-110 z-10"
          ...
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              >
                ...
                {t('contentdetail.actions.play', 'Play')}
              </button>
              <button
            ...
            <button
              ...
              title={t('contentdetail.actions.details', 'More Info')}
            >
          ...
          {content.short_description && (
            <p className="text-gray-300 text-xs sm:text-sm line-clamp-2 sm:line-clamp-3 leading-relaxed flex-1">
              {typeof content.short_description === 'string' 
                ? content.short_description 
                : t('contentdetail.messages.noDescription', 'Description not available')
              }
            </p>
          )}
        </div>
