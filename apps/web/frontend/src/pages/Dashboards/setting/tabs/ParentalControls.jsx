...
export default function ParentalControls({ user, isFamilyOwner, isFamilyMember, memberRole, familyMemberData }) {
  ...
  const handleRemoveMasterPin = async () => {
    if (verificationStep === "resetting" && currentPin.length === 4) {
      try {
        setLoading(true);
        ...
        // Then call the remove endpoint to delete PIN completely
        await api.post("/family/pin/master/remove");
        ...
        showMessage("success", 
          t('parentalControls.success.pinRemoved'), 
          t('parentalControls.success.pinRemovedDesc')
        );
        ...
        console.error("Error removing master PIN:", error);
        const errorMessage = error.response?.data?.error || t('parentalControls.errors.removePinFailed');
        showMessage("error", t('parentalControls.errors.removeFailed'), errorMessage);
        setVerificationStep("none");
      } finally {
        setLoading(false);
      }
    } else {
      // Start the remove process - ask for current PIN first
                    ...
                    <div className="max-w-md mx-auto space-y-4">
                      {!hasActivePin ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          >
                            <RotateCcw size={20} />
                            {t('parentalControls.actions.removePin')}
                          </button>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="max-w-sm mx-auto space-y-6">
                      <div className="text-center">
                        ...
                        <p className="text-gray-400 text-sm">
                          {verificationStep === "resetting" 
                            ? t('parentalControls.verification.removeDescription')
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => {
                            if (verificationStep === "changing") {
                              verifyCurrentPin();
                            } else if (verificationStep === "resetting") {
                              handleRemoveMasterPin();
                            }
                          }}
                        ...
                        >
                          ...
                          {loading 
                            ? t('parentalControls.actions.verifying') 
                            : verificationStep === "resetting" 
                              ? t('parentalControls.actions.verifyAndRemove')
                              : t('parentalControls.actions.verifyPin')
                          }
                        </button>
                        <button
            ...
            {activeTab === "insights" && kidProfiles.length === 0 && (
              <div className="text-center py-12">
                ...
                <h4 className="text-lg font-medium text-gray-300 mb-2">
                  {t('parentalControls.noKidProfiles.title')}
                </h4>
                <p className="text-gray-500 mb-4 max-w-md mx-auto">
                  {t('parentalControls.noKidProfiles.description')}
                </p>
              </div>
            )}
          </Suspense>
