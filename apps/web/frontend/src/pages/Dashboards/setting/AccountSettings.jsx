...
import { ArrowLeft, MoreHorizontal, Bell, Users, Shield, UserCog, Mail, Phone, User as UserIcon, ChevronDown, ChevronUp } from "lucide-react";
...
// Helper function to get all user identifiers
const getUserIdentifiers = (userData) => {
  if (!userData) return [];
  ...
  if (userData.phone) {
    identifiers.push({
      ...
      verified: userData.phone_verified || false
    });
  }
  ...
  if (userData.username) {
    identifiers.push({
      ...
      verified: userData.username_verified || true
    });
  }
  
  // Full name identifier (if different from display name)
  const fullName = userData.first_name || userData.last_name 
    ? `${userData.first_name || ''} ${userData.last_name || ''}`.trim()
    : null;
  
  if (fullName && fullName !== getUserDisplayName(userData)) {
    identifiers.push({
      type: 'name',
      value: fullName,
      icon: <UserIcon className="w-3.5 h-3.5" />,
...
export default function AccountSettings() {
  ...
  const [showUserIdentifiers, setShowUserIdentifiers] = useState(false);
  ...
  // Calculate user identifiers
  const userIdentifiers = useMemo(() => getUserIdentifiers(user), [user]);
  const displayName = useMemo(() => getUserDisplayName(user), [user]);
  const hasMultipleIdentifiers = userIdentifiers.length > 1;
  const primaryIdentifier = userIdentifiers[0] || null;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex flex-col min-w-0 flex-1">
            ...
            {/* User Info Display - Enhanced with multiple identifiers */}
            <div className="mt-2">
              <div className="flex items-center gap-2 text-gray-300 text-sm">
                ...
                {/* Show more identifiers button if available */}
                {hasMultipleIdentifiers && (
                  <button
                    onClick={() => setShowUserIdentifiers(!showUserIdentifiers)}
                    className="ml-2 flex items-center gap-1 text-[#BC8BBC] hover:text-[#9b69b2] text-xs transition"
                  >
                    {showUserIdentifiers ? (
                      <>
                        <ChevronUp size={12} />
                        {t("accountInfo.hideIdentifiers", "Hide")}
                      </>
                    ) : (
                      <>
                        <ChevronDown size={12} />
                        {t("accountInfo.showIdentifiers", "More")}
                      </>
                    )}
                  </button>
                )}
              ...
              {/* Additional identifiers dropdown */}
              {showUserIdentifiers && userIdentifiers.length > 1 && (
                <div className="mt-2 bg-gray-900/70 border border-gray-700 rounded-lg p-3 animate-fade-in">
                  <div className="space-y-2">
                    {userIdentifiers.slice(1).map((identifier, index) => (
                      <div 
                        key={`${identifier.type}-${index}`}
                        className="flex items-center justify-between p-2 bg-gray-800/50 rounded hover:bg-gray-800 transition"
                      >
                        <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${
                          ...
                          {identifier.verified 
                            ? t("accountInfo.verified", "Verified") 
                            : t("accountInfo.notVerified", "Not Verified")}
                        </span>
                      </div>
                    ))}
                  </div>
                  <p className="text-gray-400 text-xs mt-2 italic">
                    {t("accountInfo.identifiersNote", "You can manage these identifiers in Account settings")}
                  </p>
                </div>
              )}
            </div>
