...
const PaymentPage = () => {
  ...
  const [redirectUrl, setRedirectUrl] = useState('https://oliviuus.com'); // Default main site
  ...
  const MAIN_SITE_URL = import.meta.env.VITE_MAIN_SITE_URL || 'https://oliviuus.com';
  ...
  useEffect(() => {
    ...
    fetchUserInfo();
  }, [plan, navigate, location.search, MAIN_SITE_URL]);
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        if (userData) {
          ...
          // 🚨 Auto-fill form with user data - handle flexible user model
          ...
          setFormData({
            ...
            // Use email if available, otherwise use phone or username
            customerEmail: userData.email ||
              (userData.phone ? `${userData.phone}@mobile.user` :
                (userData.username ? `${userData.username}@oliviuus.com` : ''))
          });
        } else {
          // User data is empty - treat as not authenticated
  ...
  // Function to redirect to login with proper redirect URL
  const redirectToLogin = () => {
    const currentPath = window.location.pathname + window.location.search;
    const loginUrl = `${ACCOUNT_CENTER_URL}/auth?redirect=${encodeURIComponent(window.location.href)}`;
  ...
  const validateForm = () => {
    ...
    // Validate email only if it exists in form
    if (formData.customerEmail && !formData.customerEmail.includes('@')) {
      newErrors.customerEmail = t('userSubscription.validation.emailInvalid');
    }
  ...
  const handleInputChange = (field, value) => {
    // 🛡️ Prevent editing email field if user has email from DB
    if (field === 'customerEmail' && userInfo?.email) {
      return; // Don't allow email modification for users with email
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Only include email if it exists (not a placeholder)
      if (formData.customerEmail && formData.customerEmail.includes('@')) {
        paymentData.customerEmail = formData.customerEmail;
      }
  ...
  // 🚨 Check if user is authenticated - show loading or redirect
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <Loader className="w-12 h-12 animate-spin text-[#BC8BBC] mx-auto mb-4" />
          <h2 className="text-xl font-bold text-white mb-2">{t('userSubscription.loading.title')}</h2>
          <p className="text-gray-400">{t('userSubscription.loading.message')}</p>
        </div>
      </div>
    );
  }
        ...
        {userInfo && (
          <div className="bg-gray-800 rounded-lg p-4 mb-6 border border-gray-700">
            ...
            <div className="space-y-2 text-sm">
              ...
              {userInfo.phone && (
                <div className="flex justify-between">
                  <span className="text-gray-400">{t('userSubscription.userInfo.phone')}:</span>
              ...
              {userInfo.username && (
                <div className="flex justify-between">
                  <span className="text-gray-400">{t('userSubscription.userInfo.username')}:</span>
              ...
              {(userInfo.first_name || userInfo.last_name) && (
                <div className="flex justify-between">
                  <span className="text-gray-400">{t('userSubscription.userInfo.fullName')}:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                readOnly={!!userInfo?.email} // Only read-only if user has email in DB
                onChange={(e) => !userInfo?.email && handleInputChange('customerEmail', e.target.value)}
                className={`w-full px-4 py-3 rounded-lg text-white focus:outline-none focus:ring-2 focus:border-transparent ${userInfo?.email
                  ? 'bg-gray-700 border-gray-600 cursor-not-allowed'
                  : 'bg-gray-800 border-gray-700 focus:ring-[#BC8BBC]'}`}
                placeholder={userInfo?.email ? userInfo.email : t('userSubscription.paymentForm.emailPlaceholder')}
              />
              <p className="text-xs text-gray-400 mt-1">
                {userInfo?.email
                  ? t('userSubscription.paymentForm.emailDescriptionLocked')
                  : t('userSubscription.paymentForm.emailDescriptionOptional')}
              </p>
