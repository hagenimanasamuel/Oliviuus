...
const UserInfoStep = () => {
  ...
  const { loginUser, refreshUser } = useAuth(); // Get loginUser and refreshUser from auth context
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        // ✅ Step 2: Immediately log the user in after signup
        ...
        if (loginResponse.status === 200) {
          // ✅ Step 3: Fetch user data and update auth context
          const userResponse = await api.get("/auth/me", { withCredentials: true });
          
          // Use the loginUser function from AuthContext to properly update state
          loginUser(userResponse.data);
          
          // Optional: Force refresh user data to ensure everything is synchronized
          await refreshUser();
          
          // ✅ Step 4: Redirect to subscription page after successful login
          navigate("/subscription", { replace: true });
        } else {
          setError(t("userInfo.auto_login_failed"));
        }
      } else {
      ...
      if (err.response?.status === 400) {
        setError(err.response.data.error || t("userInfo.invalid_data"));
      } else if (err.response?.status === 409) {
        setError(t("userInfo.user_exists"));
      } else {
        setError(t("userInfo.server_error"));
      }
    } finally {
