...
const CustomNameStep = ({ initialData, onSubmit, loading, isDarkMode, onBack }) => {
  ...
  // Blocked names list (case-insensitive)
  const BLOCKED_NAMES = [
    // Brand names
    ...
    'admin', 'administrator', 'superadmin', 'root', 'system', 'sysadmin',
    'moderator', 'support', 'help', 'customer', 'client', 'user', 'users',
    'official', 'team', 'staff', 'employee', 'staffmember',
    
    // Common suspicious names
    'test', 'testing', 'tester', 'demo', 'sample', 'sampling', 'sampler',
    'example', 'exemplar', 'specimen',
    'guest', 'visitor', 'anonymous', 'anon', 'unknown',
    'fake', 'dummy', 'placeholder', 'temp', 'temporary',
    
    // Generic/placeholder names
    'john', 'jane', 'doe', 'smith', 'user1', 'user2', 'user3',
    'firstname', 'lastname', 'fullname', 'name', 'username',
    
    // Celebrity/character names
    'superman', 'batman', 'spiderman', 'ironman', 'captain',
    'celebrity', 'famous', 'star',
    
    // Offensive/inappropriate base words
    'stupid', 'idiot', 'dumb', 'fakeuser', 'bogus',
    
    // Tech/bot names
    'bot', 'robot', 'ai', 'assistant', 'chatbot', 'automation'
  ];
  ...
  const validateName = (name, fieldName) => {
    if (!name.trim()) {
      ...
      if (fieldName === 'lastName') return "Last name is required"; // Changed to required
      return "";
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      onSubmit({ 
        ...
        lastName: lastName.trim() // Now required, will never be empty
      });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        ...
        <div>
          <div className="relative">
            ...
            {/* Floating label - Changed from (optional) to * */}
            <label
              className={`absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 pointer-events-none transition-all duration-200 ${
                isDarkMode 
                  ? 'text-gray-400 peer-focus:text-purple-400' 
            ...
            >
              {lastName || lastNameFocused ? "Last name *" : "Last name *"} {/* Changed from optional */}
            </label>
      ...
      <p className={`text-xs text-center mt-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
        Use your real first and last name {/* Updated message */}
      </p>
    </form>
  );
};

export default CustomNameStep;