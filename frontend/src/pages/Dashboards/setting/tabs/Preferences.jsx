...
export default function Preferences({ user }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update UI instantly and refresh browser
      i18n.changeLanguage(language);
      
      // Refresh browser immediately to apply language changes
  ...
  return (
    <div className="space-y-6 max-w-md mx-auto p-2">
      ...
      {message && (
        <div className="text-red-400 text-center mt-2 p-2 rounded bg-red-900/20">
          <p>{message}</p>
        </div>
      )}
    </div>
