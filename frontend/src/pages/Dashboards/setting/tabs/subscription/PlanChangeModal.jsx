...
export default function PlanChangeModal({ currentSubscription, onClose, onSuccess }) {
  ...
  const fetchAvailableUpgrades = async () => {
    ...
    try {
      ...
      setErrors({ fetch: 'Failed to load upgrade options. Please refresh the page.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePlanSelection = async (plan) => {
    ...
    try {
      ...
      setErrors({ 
        submit: error.response?.data?.message || 'Failed to calculate upgrade cost' 
      });
  ...
  const processUpgradePayment = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Upgrade payment failed');
      }
    } catch (error) {
      console.error('Upgrade payment failed:', error);
      setErrors({
        submit: error.response?.data?.message || 'Failed to process upgrade payment'
      });
      setPaymentStep('payment-details');
    } finally {
      setUpgrading(false);
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (upgradeStatus.upgradeCompleted) {
          ...
          setErrors({ submit: 'Upgrade payment failed. Please try again.' });
          setPaymentStep('payment-details');
          clearInterval(pollingRef.current);
        }
      } catch (error) {
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-[#BC8BBC]/10 to-purple-600/10 border border-[#BC8BBC] rounded-lg p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-white">Secure Plan Upgrade</h4>
            <p className="text-gray-300 text-sm">
              All transactions are encrypted and monitored for security. Your payment details are protected.
            </p>
          </div>
        </div>
      </div>
      ...
      {currentSubscription && (
        <div className="bg-[#BC8BBC]/10 border border-[#BC8BBC] rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-semibold text-white">Current Plan: {currentSubscription.subscription_name}</h4>
              <p className="text-gray-300 text-sm">
                Upgrade to a higher plan for immediate access with prorated pricing.
              </p>
            </div>
          </div>
        </div>
      ...
      {upgradePlans.length === 0 ? (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-6 text-center">
          ...
          <h4 className="font-semibold text-yellow-400 mb-2">You're on the Highest Plan</h4>
          <p className="text-yellow-300 text-sm">
            You're currently on our highest tier plan. There are no available upgrades at this time.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                ...
                <p className="text-green-300 text-xs">
                  Click to calculate prorated cost
                </p>
              </div>
            </div>
          ))}
  ...
  const renderCalculating = () => (
    <div className="text-center space-y-6">
      ...
      <div>
        <h3 className="text-xl font-bold text-white mb-2">Calculating Upgrade</h3>
        <p className="text-gray-400">
          Calculating your prorated upgrade cost...
        </p>
      </div>
    </div>
  );
      ...
      <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-green-400 text-sm">Plan Upgrade</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-white">
              {upgradeCalculation ? formatCurrency(upgradeCalculation.finalAmount) : 'Calculating...'}
            </p>
            <p className="text-green-300 text-sm">Prorated Upgrade</p>
          </div>
        </div>
        ...
        {upgradeCalculation && (
          <div className="mt-4 p-4 bg-gray-700/50 rounded-lg border border-gray-600">
            <h4 className="font-semibold text-white text-sm mb-3">Cost Breakdown:</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-400">Base Upgrade:</span>
                <span className="text-white">{formatCurrency(upgradeCalculation.baseUpgradeAmount)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Operational Adjustment (10%):</span>
            ...
            <p className="text-gray-400 text-xs mt-3">
              Based on {upgradeCalculation.daysRemaining} remaining days in your billing cycle
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mt-4">
          ...
          <div>
            ...
            <p className="text-white">{upgradeCalculation?.daysRemaining || getDaysRemaining()} days</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); processUpgradePayment(); }} className="space-y-4">
        ...
        <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
          ...
          <p className="text-gray-400 text-xs mt-2">
            Amount is securely calculated on our servers and cannot be modified
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-[#BC8BBC]/20 to-purple-600/20 border border-[#BC8BBC]/30 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div>
              <h4 className="font-semibold text-[#BC8BBC] text-sm mb-1">Secure Upgrade Payment</h4>
  ...
  const renderProcessing = () => (
    <div className="text-center space-y-6">
      ...
      <div>
        <h3 className="text-xl font-bold text-white mb-2">Processing Upgrade</h3>
        <p className="text-gray-400">
          Your plan upgrade is being securely processed...
        </p>
      </div>
    ...
    <div className="text-center space-y-6">
      ...
      <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
        <p className="text-green-300 text-sm">
          ✅ Upgrade complete • 🔒 Payment secured • 🚀 Features activated
        </p>
      </div>
    </div>
  );
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-800 rounded-xl p-8 max-w-md w-full">
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <h3 className="text-lg font-semibold text-white">Loading Upgrade Options...</h3>
            <p className="text-gray-400 text-center">Fetching available upgrade plans</p>
          </div>
        </div>
      </div>
    );
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-2xl font-bold text-white">
                {paymentStep === 'select-plan' && 'Upgrade Your Plan'}
                {paymentStep === 'calculating' && 'Calculating Upgrade'}
                {paymentStep === 'payment-details' && 'Complete Upgrade Payment'}
                {paymentStep === 'processing' && 'Processing Upgrade'}
                {paymentStep === 'success' && 'Upgrade Successful'}
              </h3>
              <p className="text-gray-400 mt-1">
                {paymentStep === 'select-plan' && 'Choose a higher plan to upgrade instantly'}
                {paymentStep === 'calculating' && 'Calculating your prorated upgrade cost'}
                {paymentStep === 'payment-details' && 'Enter your payment details to complete the upgrade'}
                {paymentStep === 'processing' && 'Your upgrade is being processed'}
                {paymentStep === 'success' && 'Your plan has been upgraded successfully'}
              </p>
            </div>
