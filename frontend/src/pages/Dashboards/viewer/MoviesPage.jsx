...
import { Film, Grid, List, Play, Clock, TrendingUp, Sparkles, Filter, X, Volume2, VolumeX, Plus, Heart, Info, Check, Loader2, ChevronDown } from 'lucide-react';
...
const MoviesPage = () => {
    ...
    const fetchMovies = useCallback(async () => {
        try {
            ...
            if (response.data.success) {
                const moviesData = response.data.data.contents || [];
                
                const onlyMovies = moviesData.filter(content => 
                    content.content_type === 'movie' || content.type === 'movie'
                );
                
                setMovies(onlyMovies);
                setFilteredMovies(onlyMovies);
                
                const featured = onlyMovies
            ...
            setError(t('movies.errors.unableToLoad'));
            setMovies([]);
            setFilteredMovies([]);
            setFeaturedMovie(null);
        } finally {
    ...
    useEffect(() => {
        const fetchHeroPreferences = async () => {
            ...
            try {
                const response = await userPreferencesApi.getUserContentPreferences(featuredMovie.id);
                if (response.success) {
    ...
    useEffect(() => {
        ...
        filtered = filtered.filter(movie => 
            movie.content_type === 'movie' || movie.type === 'movie'
        );
    ...
    useEffect(() => {
        if (featuredMovie?.trailer && !heroLoading) {
    ...
    }, [featuredMovie, heroLoading]);

    const startTrailerPlayback = async () => {
        if (!featuredMovie?.trailer || !videoRef.current) return;
    ...
    const toggleTrailer = () => {
        if (!featuredMovie?.trailer) return;
    ...
    const handlePlayFeatured = () => {
        if (featuredMovie) {
            handlePlayContent(featuredMovie);
        }
    };

    const handleFeaturedMoreInfo = () => {
        if (featuredMovie) {
            openDetailModal(featuredMovie);
        }
    };

    const genres = [...new Set(movies
        .flatMap(movie => movie.genres?.map(g => g.name) || [])
    )].sort();
            ...
            <div className="min-h-screen bg-gray-900 pt-16 flex items-center justify-center px-4">
                <div className="text-center max-w-md w-full">
                    ...
                    <h3 className="text-white text-xl font-semibold mb-2">
                        {t('movies.errors.unableToLoadTitle')}
                    </h3>
                ...
                >
                    ...
                    >
                        <div className="relative w-full h-full">
                            <img
                                src={featuredMovie.media_assets?.[0]?.url || '/api/placeholder/1920/1080'}
                    ...
                    <div className="relative z-20 h-full flex items-center px-4 sm:px-6 lg:px-8">
                        <div className="max-w-2xl text-left w-full">
                            ...
                            <p className="text-gray-200 text-base sm:text-lg md:text-xl mb-6 sm:mb-8 leading-relaxed max-w-3xl drop-shadow-lg line-clamp-3 sm:line-clamp-none">
                                {featuredMovie.short_description || featuredMovie.description}
                            </p>
                            ...
                            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                                ...
                                <button
                                    onClick={(e) => handleAddToLibrary(featuredMovie, e)}
                                ...
                                <button
                                    onClick={(e) => handleLikeContent(featuredMovie, e)}
                                    className={`flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg font-bold text-sm sm:text-base transition-all duration-200 border backdrop-blur-sm transform hover:scale-105 ${
                                        heroPreferences.isLiked
                                ...
                                {featuredMovie.trailer && isTrailerPlaying && (
                                    <button
                                        onClick={toggleMute}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        `}>
                            <div className="flex flex-col md:flex-row md:flex-wrap items-stretch md:items-center gap-3 w-full md:w-auto">
                                ...
                                <div className="relative group w-full md:w-auto">
                                    ...
                                    >
                                        <option value="">
                                            {t('movies.filters.allCategories')}
                                        </option>
                                        {genres.map(genre => (
                                ...
                                <div className="relative group w-full md:w-auto">
                                    ...
                                    >
                                        <option value="latest">
                                            {t('movies.filters.sort.latest')}
                                        </option>
                                        <option value="oldest">
                                            {t('movies.filters.sort.oldest')}
                                        </option>
                                        <option value="popular">
                                            {t('movies.filters.sort.popular')}
                                        </option>
                                    </select>
                                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                    ...
                    <div className="mb-6">
                        <p className="text-gray-400 text-sm">
                            {t('movies.resultsCount', { count: filteredMovies.length })}
                        </p>
                    </div>
                    ...
                    {filteredMovies.length === 0 ? (
                        <div className="text-center py-16 sm:py-24">
                            ...
                            <h3 className="text-white text-lg sm:text-xl lg:text-2xl font-semibold mb-2 sm:mb-3">
                                {t('movies.emptyState.title')}
                            </h3>
                            <p className="text-gray-400 mb-6 max-w-md mx-auto text-sm sm:text-base px-4">
                                {movies.length === 0 
                                    ? t('movies.emptyState.noMoviesAvailable') 
                                    : t('movies.emptyState.adjustFilters')
                                }
                            </p>
                            {movies.length > 0 && (
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-4 sm:gap-6">
                                        <div className="relative flex-shrink-0">
                                            <img 
                                                src={movie.media_assets?.[0]?.url || '/api/placeholder/120/180'} 
                                                alt={movie.title}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="text-white font-bold text-lg sm:text-xl mb-2 group-hover:text-[#BC8BBC] transition-colors truncate">{movie.title}</h3>
                                            ...
                                            <div className="flex items-center gap-3 sm:gap-4 mb-3 text-xs sm:text-sm text-gray-400">
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    {movie.duration_minutes || t('contentdetail.metadata.notAvailable')} min
                                                </span>
                                                <span>•</span>
