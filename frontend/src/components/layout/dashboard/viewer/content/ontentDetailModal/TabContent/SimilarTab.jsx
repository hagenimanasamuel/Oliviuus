...
const SimilarTab = ({ similarContent, contentData }) => {
  ...
  const getContentBadge = (item) => {
    if (item.content_type === 'series') {
      return { label: 'Series', color: 'bg-purple-500' };
    } else if (item.content_type === 'movie') {
      return { label: 'Movie', color: 'bg-blue-500' };
    } else if (item.content_type === 'documentary') {
      return { label: 'Doc', color: 'bg-green-500' };
    } else {
      return { label: 'Content', color: 'bg-gray-500' };
    }
  };
    ...
    return (
      <div>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-xl font-bold text-white">Finding Similar Content...</h3>
        </div>
        <SkeletonLoader />
      </div>
    );
  ...
  if (displayContent.length === 0) {
    return (
      <div className="text-center py-12">
        <Film className="w-16 h-16 text-gray-500 mx-auto mb-4" />
        <h3 className="text-xl font-bold text-white mb-2">No Similar Content Found</h3>
        <p className="text-gray-400 mb-6">
          We're working on adding more content that matches your interests.
        </p>
        <button
          onClick={() => navigate('/browse')}
