...
const MediaTab = ({ contentData }) => {
  ...
  const mediaCategories = [
    {
      ...
      description: 'Watch previews and promotional videos'
    },
    {
      key: 'behindScenes',
    ...
    },
    {
      ...
      description: 'Official posters and key artwork'
    },
    {
      key: 'photos',
      label: 'Photos & Stills',
      icon: <Image className="w-5 h-5" />,
      description: 'Production stills and screenshots'
    }
  ...
  const getCategoryDisplayName = (category) => {
    const names = {
      ...
      'posters': 'Posters & Artwork',
      'photos': 'Photos & Stills'
    };
    ...
    if (isPoster) {
      return (
        <div className="w-full h-full bg-gradient-to-br from-gray-800 to-gray-900 flex flex-col items-center justify-center p-4">
          ...
          <div className="text-center">
            <div className="text-white font-semibold text-sm mb-1">
              {asset.asset_title || 'Movie Poster'}
            </div>
            <div className="text-gray-400 text-xs">
              {contentData?.title}
            </div>
      ...
      >
        ...
        <div className={`${isPoster ? 'aspect-[2/3]' : 'aspect-video'} relative overflow-hidden bg-gray-800`}>
          {!hasError ? (
            <>
              <img
                src={asset.url}
                alt={asset.asset_title || `${getCategoryDisplayName(category)} from ${contentData?.title}`}
        ...
        <div className="p-4">
          <h4 className="text-white font-semibold text-sm mb-2 line-clamp-2 group-hover:text-[#BC8BBC] transition-colors duration-300">
            {asset.asset_title || `${getCategoryDisplayName(category)} Content`}
          </h4>
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-400 text-lg animate-slide-up">
          Discover exclusive content from {contentData?.title}
        </p>
      </div>
          ...
          {mediaCategories.map((category, index) => (
            ...
            >
              ...
              <div className="mt-2 text-xs opacity-60">
                {mediaData[category.key]?.length || 0} items
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 p-6 animate-slide-up">
        {currentMedia.length > 0 ? (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between animate-fade-in">
              ...
              <div className="text-[#BC8BBC] font-semibold animate-slide-left">
                {currentMedia.length} {currentMedia.length === 1 ? 'item' : 'items'}
              </div>
            </div>
          ...
          <div className="text-center py-12 animate-fade-in">
            ...
            <h4 className="text-white text-lg font-semibold mb-2">No Content Available</h4>
            <p className="text-gray-400">Check back later for exclusive {getCategoryDisplayName(activeCategory).toLowerCase()}.</p>
          </div>
        )}
      </div>
      ...
      {Object.values(mediaData).flat().length > 0 && (
        <div className="text-center animate-fade-in">
          <div className="bg-gradient-to-r from-[#BC8BBC]/10 to-purple-600/10 rounded-2xl border border-[#BC8BBC]/20 p-6 transform hover:scale-105 transition-all duration-500">
            ...
            <h4 className="text-white font-semibold text-lg mb-2">Explore More</h4>
            <p className="text-gray-300">
              Discover all the exclusive content available for {contentData?.title}
            </p>
          </div>
        </div>
      )}
              ...
              {['trailer', 'teaser', 'episodeTrailer'].includes(selectedMedia.asset_type) ? (
                <div className="aspect-video bg-black">
                  ...
                  <div className="absolute inset-0 hidden flex-col items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900">
                    ...
                    <div className="text-white font-semibold text-lg">Video Not Available</div>
                    <div className="text-gray-400 text-sm mt-1">Try again later</div>
                  </div>
                </div>
              ) : (
                <div 
                ...
                >
                  {!isImageError(selectedMedia.id) ? (
                    <img
                      src={selectedMedia.url}
                      alt={selectedMedia.asset_title || `Media from ${contentData?.title}`}
                    ...
                    <div className="w-full h-96 flex flex-col items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900">
                      ...
                      <div className="text-white font-semibold text-lg">Image Not Available</div>
                      <div className="text-gray-400 text-sm mt-1">{selectedMedia.asset_title || 'Media Content'}</div>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="p-6 border-t border-white/10">
              <div className="space-y-3 animate-fade-in">
                <h3 className="text-xl font-bold text-white">
                  {selectedMedia.asset_title || `Exclusive ${getCategoryDisplayName(activeCategory).toLowerCase()}`}
                </h3>
