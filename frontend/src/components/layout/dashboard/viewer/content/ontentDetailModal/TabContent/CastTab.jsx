...
const CastTab = ({ contentData }) => {
  ...
  const getRoleDisplayName = (role) => {
    const roleNames = {
      'director': t('contentdetail.roles.director', 'Director'),
      'producer': t('contentdetail.roles.producer', 'Producer'),
      'writer': t('contentdetail.roles.writer', 'Writer'),
      'cinematographer': t('contentdetail.roles.cinematographer', 'Cinematographer'),
      'composer': t('contentdetail.roles.composer', 'Music Composer'),
      'editor': t('contentdetail.roles.editor', 'Editor')
    };
    return roleNames[role] || role.charAt(0).toUpperCase() + role.slice(1);
  };

  const getCreditTypeDisplay = (creditType) => {
    const creditTypes = {
      'supporting': t('contentdetail.creditTypes.supporting', 'Supporting'),
      'lead': t('contentdetail.creditTypes.lead', 'Lead'),
      'guest': t('contentdetail.creditTypes.guest', 'Guest'),
      'cameo': t('contentdetail.creditTypes.cameo', 'Cameo')
    };
    return creditTypes[creditType] || creditType;
  };
    ...
    <div className="space-y-8">
      ...
      <div className="text-center">
        <h3 className="text-3xl font-bold text-white mb-3 flex items-center justify-center gap-3">
          ...
          {t('contentdetail.sections.castAndCrew', 'Cast & Crew')}
        </h3>
        <p className="text-gray-400 text-lg">
          {t('contentdetail.messages.meetTheTeam', 'Meet the talented people behind {{title}}', { title: contentData?.title })}
        </p>
      </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h4 className="text-xl font-bold text-white">
              {t('contentdetail.sections.cast', 'Cast')}
            </h4>
            <p className="text-gray-400 text-sm">
              {t('contentdetail.messages.talentedActors', '{{count}} talented actors', { count: getCast().length })}
            </p>
          </div>
        </div>
              ...
              <div className="space-y-1">
                ...
                {person.character_name && (
                  <div className="mt-2">
                    <p className="text-gray-400 text-xs italic leading-tight">
                      {t('contentdetail.messages.asCharacter', 'as')}
                    </p>
                ...
                {person.credit_type && person.credit_type !== 'supporting' && (
                  <span className="inline-block mt-2 px-2 py-1 bg-[#BC8BBC]/20 text-[#BC8BBC] text-xs rounded-full border border-[#BC8BBC]/30">
                    {getCreditTypeDisplay(person.credit_type)}
                  </span>
                )}
              </div>
            </div>
        ...
        {getCast().length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400">
              {t('contentdetail.errors.noCastInfo', 'No cast information available')}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h4 className="text-xl font-bold text-white">
              {t('contentdetail.sections.productionCrew', 'Production Crew')}
            </h4>
            <p className="text-gray-400 text-sm">
              {t('contentdetail.messages.creativeMinds', 'The creative minds behind the production')}
            </p>
          </div>
        </div>
        ...
        {!['director', 'producer', 'writer', 'cinematographer', 'composer', 'editor'].some(role => getCrew(role).length > 0) && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400">
              {t('contentdetail.errors.noCrewInfo', 'No crew information available')}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-gradient-to-r from-[#BC8BBC]/10 to-purple-600/10 rounded-2xl border border-[#BC8BBC]/20 p-6 text-center">
        ...
        <h4 className="text-white font-semibold text-lg mb-2">
          {t('contentdetail.sections.fullCastCrew', 'Full Cast & Crew')}
        </h4>
        <p className="text-gray-300 text-sm">
          {t('contentdetail.messages.exploreMore', 'Explore more about the talented individuals who brought this story to life')}
        </p>
      </div>
    </div>
  );
