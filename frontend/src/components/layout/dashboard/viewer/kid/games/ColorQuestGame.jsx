...
const ColorQuestGame = ({ onGameEvent, isFullscreen }) => {
  ...
  const colorPalette = [
    { name: t('colorQuestGame.colors.red'), hex: '#FF6B6B', emoji: '🔴', lightHex: '#FFB3B3' },
    { name: t('colorQuestGame.colors.blue'), hex: '#4ECDC4', emoji: '🔵', lightHex: '#A6E3E9' },
    { name: t('colorQuestGame.colors.yellow'), hex: '#FFD166', emoji: '🟡', lightHex: '#FFE8A5' },
    { name: t('colorQuestGame.colors.green'), hex: '#06D6A0', emoji: '🟢', lightHex: '#8CFFE1' },
    { name: t('colorQuestGame.colors.purple'), hex: '#A13E97', emoji: '🟣', lightHex: '#D9A8D9' },
    { name: t('colorQuestGame.colors.orange'), hex: '#FF9A76', emoji: '🟠', lightHex: '#FFC8B4' },
    { name: t('colorQuestGame.colors.pink'), hex: '#FF6B9D', emoji: '💖', lightHex: '#FFB8D9' },
    { name: t('colorQuestGame.colors.brown'), hex: '#8B4513', emoji: '🟤', lightHex: '#D2691E' },
  ];

  const colorMixes = {
    [`${t('colorQuestGame.colors.red')} + ${t('colorQuestGame.colors.blue')}`]: t('colorQuestGame.colors.purple'),
    [`${t('colorQuestGame.colors.blue')} + ${t('colorQuestGame.colors.yellow')}`]: t('colorQuestGame.colors.green'),
    [`${t('colorQuestGame.colors.red')} + ${t('colorQuestGame.colors.yellow')}`]: t('colorQuestGame.colors.orange'),
    [`${t('colorQuestGame.colors.red')} + White`]: t('colorQuestGame.colors.pink'),
    [`${t('colorQuestGame.colors.blue')} + White`]: t('colorQuestGame.colors.lightBlue'),
    [`${t('colorQuestGame.colors.yellow')} + ${t('colorQuestGame.colors.blue')} + ${t('colorQuestGame.colors.red')}`]: t('colorQuestGame.colors.brown'),
  };

  const colorObjectsList = [
    { color: t('colorQuestGame.colors.red'), items: ['🍎', '🚗', '❤️', '🎈', '🍓'] },
    { color: t('colorQuestGame.colors.blue'), items: ['🌊', '🐳', '💙', '🦋', '🔵'] },
    { color: t('colorQuestGame.colors.yellow'), items: ['🌻', '🍋', '⭐', '🌞', '🦆'] },
    { color: t('colorQuestGame.colors.green'), items: ['🌿', '🐸', '🍏', '🐢', '🥦'] },
    { color: t('colorQuestGame.colors.purple'), items: ['🍇', '👑', '🟣', '🍆', '🌂'] },
    { color: t('colorQuestGame.colors.orange'), items: ['🍊', '🎃', '🦊', '🔥', '🧡'] },
  ];

  const characters = [
    { emoji: '🎨', name: t('colorQuestGame.characters.arty.name') },
    { emoji: '🦄', name: t('colorQuestGame.characters.rainbow.name') },
    { emoji: '🐙', name: t('colorQuestGame.characters.inky.name') },
    { emoji: '🦚', name: t('colorQuestGame.characters.peacock.name') }
  ];

  // Translated funny messages
  const funnyMessages = [
    t('colorQuestGame.feedback.correct.colorGenius'),
    t('colorQuestGame.feedback.correct.perfectMatch'),
    t('colorQuestGame.feedback.correct.youreRainbow'),
    t('colorQuestGame.feedback.correct.masterPainter'),
    t('colorQuestGame.feedback.correct.colorWizard'),
    t('colorQuestGame.feedback.correct.bullseye')
  ];
  ...
  const generateColorQuestion = () => {
    ...
    setCurrentQuestion({
      ...
      question: t('colorQuestGame.questions.identify.default'),
  ...
  const generateMixingQuestion = () => {
    ...
    const color1Obj = colorPalette.find(c => c.name === color1) || { name: color1, hex: '#FFFFFF' };
    const color2Obj = colorPalette.find(c => c.name === color2) || { name: color2, hex: '#FFFFFF' };
    ...
    setCurrentQuestion({
      ...
      color2: color2Obj,
      result: resultObj,
      question: t('colorQuestGame.questions.mix.default', { color1, color2 })
    });
  ...
  const generateSortingObjects = () => {
    ...
    setCurrentQuestion({
      ...
      question: t('colorQuestGame.questions.sort.default', { color: targetColor.color.toLowerCase() })
    });
  };
  ...
  const generatePaintQuestion = () => {
    ...
    setCurrentQuestion({
      type: 'paint',
      targetColor: targetColor,
      question: t('colorQuestGame.questions.paint.default', { color: targetColor.name.toLowerCase() })
    });
  };
  ...
  const checkAnswer = (correct, selectedColor = null) => {
    ...
    if (correct) {
      ...
      if (streak + 1 >= 5) {
        ...
        setLives(newLives);
        setFeedback(prev => `${prev} ${t('colorQuestGame.feedback.bonusHeart')}`);
      ...
      if ((score + 1) % 5 === 0) {
        setLevel(prev => {
          ...
          setFeedback(prev => `${prev} ${t('colorQuestGame.feedback.levelUp')}`);
          return newLevel;
        });
      }
      ...
      let errorMessage = t('colorQuestGame.feedback.incorrect.tryAgain');
      if (selectedColor && gameMode === 'identify') {
        errorMessage += ` ${t('colorQuestGame.feedback.incorrect.identifyHint', { color: currentQuestion.targetColor.name })}`;
      } else if (gameMode === 'mix') {
        errorMessage += ` ${t('colorQuestGame.feedback.incorrect.mixHint')}`;
      }
      ...
      <div className={`flex items-center justify-center ${isFullscreen ? 'h-full' : 'min-h-[400px]'} p-3 w-full`}>
        <div className="w-full bg-gradient-to-br from-[#1A1A2E] to-[#16213E] rounded-xl shadow-lg p-4 border border-[#FF6B9D]/30">
          ...
          <h1 className="text-2xl font-bold text-white mb-2 text-center">
            {t('colorQuestGame.gameOver.title')}
          </h1>
          <p className="text-gray-300 mb-4 text-sm text-center">
            {t('colorQuestGame.gameOver.subtitle')}
          </p>
          ...
          >
            {t('colorQuestGame.gameOver.playAgain')}
          </button>
        </div>
      </div>
    );
      ...
      <div className="w-full bg-gradient-to-br from-[#1A1A2E] to-[#16213E] rounded-xl shadow-lg p-4 border border-[#FF6B9D]/30">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-sm font-bold text-white">
                {t('colorQuestGame.gameHeader.title')}
              </h1>
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-400">
                  {t('colorQuestGame.gameHeader.level', { level })}
                </span>
                {streak > 0 && (
                  <>
                    <span className="text-gray-600">•</span>
                    <span className="text-xs text-yellow-400">
                      {t('colorQuestGame.gameHeader.streak', { streak })}
                    </span>
                  </>
                )}
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={t('colorQuestGame.buttons.toggleSound')}
            >
              {soundEnabled ? 
        ...
        <div className="flex gap-1 mb-3 overflow-x-auto pb-2">
          ...
          >
            ...
            {t('colorQuestGame.gameModes.identify')}
          </button>
          <button
          ...
          >
            ...
            {t('colorQuestGame.gameModes.mix')}
          </button>
          <button
          ...
          >
            ...
            {t('colorQuestGame.gameModes.sort')}
          </button>
          <button
          ...
          >
            ...
            {t('colorQuestGame.gameModes.paint')}
          </button>
        </div>
        ...
        {gameMode === 'timed' && (
          <div className="mb-3">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-gray-400">
                {t('colorQuestGame.timer.timeLeft')}
              </span>
              <span className={`text-sm font-bold ${timeLeft <= 10 ? 'text-red-400' : 'text-green-400'}`}>
                {t('colorQuestGame.timer.seconds', { time: timeLeft })}
              </span>
            </div>
        ...
        <div className="flex items-center justify-center gap-1 mb-4">
          {[...Array(5)].map((_, i) => (
            <Heart
              ...
              aria-label={`${t('colorQuestGame.labels.lives')} ${i + 1}`}
            />
          ))}
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="text-center mb-2">
              ...
              {gameMode === 'identify' && currentQuestion.colorHex && (
                <div className="flex flex-col md:flex-row justify-center items-center gap-3 mb-3">
                  <div 
                    ...
                    aria-label={`${currentQuestion.targetColor.name} color sample`}
                  ></div>
                  <div className="text-center md:text-left">
                    <div className="text-sm text-gray-400">
                      {t('colorQuestGame.questions.identify.colorSample')}
                    </div>
              ...
              {gameMode === 'mix' && currentQuestion.color1 && currentQuestion.color2 && (
                <div className="flex flex-col md:flex-row justify-center items-center gap-2 mb-3">
                  <div 
                    ...
                    aria-label={currentQuestion.color1.name}
                  ></div>
                  ...
                  <div 
                    ...
                    aria-label={currentQuestion.color2.name}
                  ></div>
                  <div className="text-2xl">=</div>
                  <div 
                    ...
                    title={isCorrect !== null ? currentQuestion.result.name : t('colorQuestGame.questions.mix.resultUnknown')}
                    aria-label={`${t('colorQuestGame.labels.result')}: ${isCorrect !== null ? currentQuestion.result.name : t('colorQuestGame.questions.mix.resultUnknown')}`}
                  >
                    {isCorrect !== null && (
              ...
              {gameMode === 'paint' && bucketColor && (
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 mb-3">
                  ...
                  <div className="text-center">
                    <div className="text-sm text-gray-400 mb-1">
                      {t('colorQuestGame.questions.paint.colorDrops')}
                    </div>
                    <div className="flex gap-2 justify-center">
                      {drops.map(drop => (
                        <button
                          ...
                          aria-label={`Color drop: ${drop.color}`}
                        >
                          {drop.emoji}
                        </button>
                      ))}
            ...
            <div className="grid grid-cols-4 md:grid-cols-6 gap-2 mb-3">
              {colorObjects.map((obj, index) => (
                <button
                  ...
                  aria-label={`${obj.item} - ${obj.correct ? 'Correct' : 'Incorrect'} item`}
                >
                  {obj.item}
                </button>
              ))}
            ...
            <div className="absolute inset-0 pointer-events-none">
              {paintSplats.map((splat, index) => (
                <div
                  ...
                  aria-label={`Paint splat: ${splat.emoji}`}
                >
                  {splat.emoji}
                </div>
              ))}
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            >
              {t('colorQuestGame.buttons.newColor')}
            </button>
            <div className="flex gap-2">
              ...
              >
                {t('colorQuestGame.buttons.easier')}
              </button>
              <button
                onClick={() => setLevel(prev => prev + 1)}
                className="flex-1 py-2 rounded-lg bg-[#1A1A2E] border border-gray-700 text-gray-400 text-sm hover:text-white hover:border-gray-600"
              >
                {t('colorQuestGame.buttons.harder')}
              </button>
            </div>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-[#FF6B9D]/10">
            <div className="text-xs text-gray-400 mb-2 text-center">
              {t('colorQuestGame.labels.colorPalette')}
            </div>
