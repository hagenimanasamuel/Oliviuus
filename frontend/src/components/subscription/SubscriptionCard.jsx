...
const SubscriptionCard = ({ 
  ...
  return (
    <div 
      className={`relative bg-gradient-to-br ${plan.gradient} p-4 sm:p-5 lg:p-6 rounded-xl sm:rounded-2xl flex flex-col shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border-2 ${
        plan.popular ? 'border-[#BC8BBC] ring-1 sm:ring-2 ring-[#BC8BBC] ring-opacity-50' : 'border-transparent'
      } ${isSelected ? 'ring-2 sm:ring-4 ring-[#BC8BBC] ring-opacity-70' : ''} ${
        isMobile ? 'h-[580px]' : 'h-[640px] lg:h-[680px]'
      }`}
    >
      {/* Popular Badge */}
      {plan.popular && (
      ...
      <div className="mt-auto"> {/* mt-auto pushes this to bottom */}
        ...
        <button
          onClick={onSelect}
          disabled={loading}
          className={`w-full py-2 sm:py-3 px-4 sm:px-6 rounded-lg sm:rounded-xl font-semibold text-base sm:text-lg transition-all duration-200 ${
            plan.popular 
